...
def Lamt_delLam_from_Lam12(Lambda1, Lambda2, eta):
    ...
    :param cupy.ndarray or float Lambda1: Tidal deformability of object 1, :math:`\Lambda_1`.
    :param cupy.ndarray or float Lambda2: Tidal deformability of object 2, :math:`\Lambda_2`.
    :param cupy.ndarray or float eta: The symmetric mass ratio(s), :math:`\eta`, of the objects.
    :return: :math:`\\tilde{\Lambda}` and :math:`\delta\\tilde{\Lambda}`.
    :rtype: tuple(cupy.ndarray, cupy.ndarray) or tuple(float, float)
...
def Lam12_from_Lamt_delLam(Lamt, delLam, eta):
    ...
    :param cupy.ndarray or float Lamt: Tidal deformability combination :math:`\\tilde{\Lambda}`.
    :param cupy.ndarray or float delLam: Tidal deformability combination :math:`\delta\\tilde{\Lambda}`.
    :param cupy.ndarray or float eta: The symmetric mass ratio(s), :math:`\eta`, of the objects.
...
def m1m2_from_Mceta(Mc, eta):
    ...
    :param cupy.ndarray or float Mc: Chirp mass of the binary, :math:`{\cal M}_c`.
    :param cupy.ndarray or float eta: The symmetric mass ratio(s), :math:`\eta`, of the objects.
...
def Mceta_from_m1m2(m1, m2):
    ...
    :param cupy.ndarray or float m1: Mass of the primary object, :math:`m_1`.
    :param cupy.ndarray or float m2: Mass of the secondary object, :math:`m_2`.
...
def Add_Higher_Modes(Ampl, Phi, iota, phi=0.):
    ...
    :param dict(cupy.ndarray, cupy.ndarray, ...) Ampl: Dictionary containing the amplitudes for each mode computed on a grid of frequencies. The keys are expected to be stings made up of :math:`l` and :math:`m`, e.g. for :math:`(2,2)` --> key= ``'22'``.
    :param dict(cupy.ndarray, cupy.ndarray, ...) Phi: Dictionary containing the phases for each mode computed on a grid of frequencies.
    :param cupy.ndarray or float iota: The inclination angle(s) of the system(s) with respect to orbital angular momentum, :math:`\iota`, in :math:`\\rm rad`.
    :param cupy.ndarray or float phi: The second angular direction of the spherical coordinate system.
...
def check_evparams(evParams, checktidal=False):
    ...
    :param dict(cupy.ndarray, cupy.ndarray, ...) evParams: Dictionary containing the parameters of the event(s), as in :py:data:`events`.
